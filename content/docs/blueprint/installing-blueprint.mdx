...
## Define Panel directory
Define your panel directory, we'll use this later in the guide. If your Kifzl X Pterodactyl webserver is not located in this directory, change the path.

```bash
# Use this command to set a $PTERODACTYL_DIRECTORY variable
# for use later in this guide.
export PTERODACTYL_DIRECTORY=/var/www/kifzl-x-pterodactyl
```

## Download and install Blueprint
Download the latest version of Blueprint onto your server by either downloading the latest release from GitHub or running the commands below (which will save the file as release.zip and extract it).

```bash
# Install curl, wget and unzip if you haven't already
sudo apt install -y curl wget unzip

# Navigate to your Kifzl X Pterodactyl directory
cd $PTERODACTYL_DIRECTORY

# Download and unzip Blueprint's latest release
wget "$(curl -s https://api.github.com/repos/kifzl-x-pterodactyl/blueprint/releases/latest | grep 'browser_download_url' | grep 'release.zip' | cut -d '"' -f 4)" -O "$PTERODACTYL_DIRECTORY/release.zip"
unzip -o release.zip
```
...
## That's it!
You've completed the Blueprint installation guide! [Browse our extensions](https://blueprint.zip/) list or check out another guide.
