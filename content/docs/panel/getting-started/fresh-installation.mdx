...
<Callout type="error" title="OpenVZ Support">
  Kifzl X Pterodactyl does not support most OpenVZ systems due to incompatibilities with Docker.  
...
| Debian 11                            | Fully Supported | Kifzl X Pterodactyl is tested primarily on Debian. This is our preferred choice of OS |
| Debian 12                            | Fully Supported | Kifzl X Pterodactyl is tested primarily on Debian. This is our preferred choice of OS |
...
It's mandatory to use a secure password. Generate one at https://www.avast.com/random-password-generator
  ```sql
  CREATE USER 'kifzl-x-pterodactyl'@'127.0.0.1' IDENTIFIED BY 'yourPassword';
  CREATE DATABASE panel;
  GRANT ALL PRIVILEGES ON panel.* TO 'kifzl-x-pterodactyl'@'127.0.0.1' WITH GRANT OPTION;
  exit
  ```
</Callout>
...
<Callout type="warning" title="Backup Your APP_KEY">
  ...
  ```bash
  grep APP_KEY /var/www/reviactly/.env
  ```
</Callout>
...
```bash
mkdir -p /var/www/kifzl-x-pterodactyl
cd /var/www/kifzl-x-pterodactyl
```

```bash
curl -Lo panel.tar.gz https://github.com/kifzl-x-pterodactyl/panel/releases/latest/download/panel.tar.gz
tar -xzvf panel.tar.gz
chmod -R 755 storage/* bootstrap/cache/
```
...
<Callout type="warning" title="Backup Your APP_KEY">
  ...
  ```bash
  grep APP_KEY /var/www/kifzl-x-pterodactyl/.env
  ```
</Callout>
...
```bash
* * * * * php /var/www/kifzl-x-pterodactyl/artisan schedule:run >> /dev/null 2>&1
```
...
# /etc/systemd/system/reviq.service
[Unit]
Description=Kifzl X Pterodactyl Queue Worker
After=redis-server.service
...
Group=www-data
Restart=always
ExecStart=/usr/bin/php /var/www/kifzl-x-pterodactyl/artisan queue:work --queue=high,standard,low --sleep=3 --tries=3
StartLimitInterval=180
StartLimitBurst=30
RestartSec=5s
