l, to see how to configure Caddy to use DNS challenge for obtaining SSL certificates.
</Callout>
...
</Tabs>    ssl_prefer_server_ciphers on;
...
server {
    ...
    root /var/www/kifzl-x-pterodactyl/public;
    ...
    access_log off;
    error_log  /var/log/nginx/kifzl-x-pterodactyl.app-error.log error;
...
sudo ln -s /etc/nginx/sites-available/kifzl-x-pterodactyl.conf /etc/nginx/sites-enabled/kifzl-x-pterodactyl.conf
...
Now, you should paste the contents of the file below, replacing `<domain>` with your domain name being used in a file called `kifzl-x-pterodactyl.conf` and place the file in `/etc/apache2/sites-available`, or — if on RHEL, Rocky Linux, or AlmaLinux, `/etc/httpd/conf.d/`.
...
<VirtualHost *:443>
    # Replace the example <domain> with your domain name or IP address
    ServerName <domain>
    DocumentRoot "/var/www/kifzl-x-pterodactyl/public"
    ...
    <Directory "/var/www/kifzl-x-pterodactyl/public">
        Require all granted
        AllowOverride all
    </Directory>
...
sudo ln -s /etc/apache2/sites-available/kifzl-x-pterodactyl.conf /etc/apache2/sites-enabled/kifzl-x-pterodactyl.conf
...
<domain> {
    ...
    log {
        output file /var/log/caddy/kifzl-x-pterodactyl.log {
            roll_size 100MiB
            roll_keep_for 7d
        }
        level INFO
...
Now, you should paste the contents of the file below, replacing `<domain>` with your domain name being used in a file called `kifzl-x-pterodactyl.conf` and place the file in `/etc/nginx/sites-available/`, or — if on RHEL, Rocky Linux, or AlmaLinux, `/etc/nginx/conf.d/`.
<Tabs items={['With SSL', 'Without SSL']}>
  <Tab value="With SSL">
<Callout type="warning" title="Note">
  You cannot use direct IP to access your panel whilst using SSL
