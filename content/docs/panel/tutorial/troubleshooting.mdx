...
``` bash
tail -n 100 /var/www/kifzl-x-pterodactyl/storage/logs/laravel-$(date +%F).log
```
...
``` bash
tail -n 1000 /var/www/kifzl-x-pterodactyl/storage/logs/laravel-$(date +%F).log | grep "\[$(date +%Y)"
```
...
- Clear schedule cache ``php /var/www/kifzl-x-pterodactyl/artisan schedule:clear-cache``
- Check your php version ``php -v`` - [this page](https://kifzl-x-pterodactyl.dev/docs/panel/updating-the-panel#panel-version-requirements) will tell you what versions of php are supported by what versions of the panel
