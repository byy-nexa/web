...
## Creating a database for Kifzl X Pterodactyl
MySQL is a core component of Kifzl X Pterodactyl Panel but it can be confusing to setup and use if you've never done so before.
...
Next, we will create a user called `kifzl-x-pterodactyl` and allow logins from localhost which prevents any external connections
...
CREATE USER 'kifzl-x-pterodactyl'@'127.0.0.1' IDENTIFIED BY 'somePassword';
```
...
Finally, we need to tell MySQL that our kifzl-x-pterodactyl user should have access to the panel database. To do this, simply
run the command below.

``` sql
GRANT ALL PRIVILEGES ON panel.* TO 'kifzl-x-pterodactyl'@'127.0.0.1';
```
...
CREATE USER 'kifzl-x-pterodactyluser'@'127.0.0.1' IDENTIFIED BY 'somepassword';
```
...
```sql
GRANT ALL PRIVILEGES ON *.* TO 'kifzl-x-pterodactyluser'@'127.0.0.1' WITH GRANT OPTION;
```
